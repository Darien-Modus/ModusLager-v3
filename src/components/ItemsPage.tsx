...
import { Edit2, Trash2, ChevronDown, ChevronRight, MoreVertical, Plus, Check, X, Upload, Image as ImageIcon } from 'lucide-react';
...
export const ItemsPage: React.FC<ItemsPageProps> = ({ items, bookings, groups, refreshData }) => {
  ...
  const saveGroup = async () => {
    ...
    try {
      ...
      alert('Error saving group. Check console for details.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const groupedItems = groups.reduce((acc, group) => {
    acc[group.id] = items.filter(item => item.groupId === group.id);
    return acc;
  }, {} as Record<string, Item[]>);

  const ungroupedItems = items.filter(item => !item.groupId || item.groupId === '00000000-0000-0000-0000-000000000000');

  return (
    <div style={{ fontFamily: 'Raleway, sans-serif' }}>
      <h2 className="text-xl font-semibold mb-4" style={{ color: '#1F1F1F' }}>Items</h2>
      
      {/* Add Item Form */}
      <div className="p-3 border rounded mb-4" style={{ backgroundColor: '#F5F5F5', borderColor: '#575F60' }}>
        <div className="grid grid-cols-5 gap-2 mb-2">
          ...
          <div className="flex gap-1">
            {colorOptions.map(opt => (
              <button
                key={opt.id}
                type="button"
                onClick={() => setForm({ ...form, color: opt.id, image: '' })}
                className={`w-6 h-6 rounded border ${form.color === opt.id && !form.image ? 'border-2' : ''}`}
                style={{ 
                  backgroundColor: opt.id,
                  borderColor: form.color === opt.id && !form.image ? '#FFED00' : '#575F60'
                }}
                title={opt.name}
              />
            ))}
            <label className={`w-6 h-6 rounded border cursor-pointer flex items-center justify-center ${form.image ? 'border-2' : ''}`}
          ...
          <button 
            onClick={saveItem} 
            disabled={saving}
            className="px-2 py-1.5 rounded text-xs flex items-center justify-center gap-1"
      ...
      <div className="mb-3">
        {!showGroupForm ? (
          <button
            ...
            style={{ backgroundColor: '#F5F5F5', borderColor: '#575F60', color: '#1F1F1F', border: '1px solid #575F60' }}
          >
          ...
          <div className="p-3 border rounded" style={{ backgroundColor: 'white', borderColor: '#575F60' }}>
            <div className="flex gap-2">
              ...
              <button
                onClick={() => { setShowGroupForm(false); setEditingGroup(null); setGroupForm({ name: '', color: '#FFED00' }); }}
      ...
      <div className="space-y-2">
        {groups.filter(g => g.id !== '00000000-0000-0000-0000-000000000000').map(group => {
          ...
          return (
            <div key={group.id} className="border rounded" style={{ borderColor: '#575F60', backgroundColor: 'white' }}>
              <div className="flex items-center justify-between px-3 py-2" style={{ backgroundColor: '#F5F5F5' }}>
                <div className="flex items-center gap-2">
                  <button onClick={() => toggleGroup(group.id)} className="hover:opacity-70">
                    {isCollapsed ? <ChevronRight className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                  </button>
                  <div className="w-3 h-3 rounded" style={{ backgroundColor: group.color || '#FFED00' }} />
                ...
                <div className="relative group">
                  ...
                  <div className="absolute right-0 mt-1 w-32 bg-white border rounded shadow-lg hidden group-hover:block z-10" style={{ borderColor: '#575F60' }}>
                    <button onClick={() => { setEditingGroup(group.id); setGroupForm({ name: group.name, color: group.color || '#FFED00' }); setShowGroupForm(true); }} className="w-full text-left px-2 py-1.5 text-xs hover:bg-gray-100">
                      Rename
                    </button>
                    <button onClick={() => startMoving(group.id)} className="w-full text-left px-2 py-1.5 text-xs hover:bg-gray-100">
                      Move items
                    </button>
                    <button onClick={() => deleteGroup(group.id)} className="w-full text-left px-2 py-1.5 text-xs hover:bg-gray-100 text-red-600">
                      Delete
                    </button>
                  </div>
                </div>
              ...
              {!isCollapsed && (
                <div className="p-2">
                  {isMoving && (
                    <div className="mb-2 p-2 bg-blue-50 border border-blue-200 rounded flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        <select value={moveTargetGroup} onChange={e => setMoveTargetGroup(e.target.value)} className="px-2 py-1 border rounded text-xs" style={{ borderColor: '#575F60' }}>
                          <option value="">Ungrouped</option>
                          {groups.filter(g => g.id !== group.id && g.id !== '00000000-0000-0000-0000-000000000000').map(g => (
                            <option key={g.id} value={g.id}>{g.name}</option>
                          ))}
                        </select>
                        <button onClick={executeMoveItems} disabled={selectedItems.size === 0} className="p-1 rounded" style={{ backgroundColor: '#FFED00' }}>
                          <Check className="w-3 h-3" />
                        </button>
                        <button onClick={() => { setMovingItems(null); setSelectedItems(new Set()); }} className="p-1">
                          <X className="w-3 h-3" />
                        </button>
                      </div>
                    </div>
                  ...
                  {groupItems.length === 0 ? (
                    <p className="text-xs text-center py-3" style={{ color: '#575F60' }}>No items</p>
                  ) : (
                    <div className="space-y-1">
                      {groupItems.map(item => {
                        ...
                        return (
                          <div key={item.id} className="flex items-center justify-between p-2 border rounded" style={{ borderColor: '#e5e7eb' }}>
                            <div className="flex items-center gap-2">
                              {isMoving && (
        ...
        {ungroupedItems.length > 0 && (
          <div className="border rounded" style={{ borderColor: '#575F60', backgroundColor: 'white' }}>
            <div className="flex items-center gap-2 px-3 py-2" style={{ backgroundColor: '#F5F5F5' }}>
              <button onClick={() => toggleGroup('ungrouped')} className="hover:opacity-70">
                {collapsedGroups.has('ungrouped') ? <ChevronRight className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
              </button>
              <div className="w-3 h-3 rounded" style={{ backgroundColor: '#575F60' }} />
              <span className="font-medium text-xs" style={{ color: '#1F1F1F' }}>
