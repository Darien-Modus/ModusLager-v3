...
export const BookingsPage: React.FC<BookingsPageProps> = ({ bookings, items, projects, groups, refreshData }) => { 
  ...
  const filteredProjects = projects.filter(p =>  
    ...
    String(p?.client || '').toLowerCase().includes(projectSearch.toLowerCase())
  );
    ...
    <div style={{ fontFamily: "Raleway, sans-serif" }}> 
      ...
      <div className="p-6 border mb-6" style={{ backgroundColor: '#191A23', borderColor: '#191A23' }}> 
        ...
        <div className="space-y-4"> 
          ...
          <button onClick={save} className="px-6 py-3 text-sm font-medium border bg-[#FFED00] border-[#191A23]">{saving ? 'Saving...' : edit ? 'Update Booking' : 'Create Booking'}</button> 
        </div> 
      </div> 
